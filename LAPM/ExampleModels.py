...
from .LinearAutonomousPoolModel import LinearAutonomousPoolModel
...
class TwoPoolsNoFeedback(LinearAutonomousPoolModel):
    ...
    Attributes:
        Qt (SymPy matrix): Qt = :math:`e^{t\\,\\bf{A}}` is given
    """
    ...
    def __init__(self, alpha, u_1, u_2):
        ...
        Args:
            alpha (in [0,1] or SymPy expression): proportion of outflow from pool 1 that goes to pool 2
        ...
        t = symbols('t')
        self.Qt = Matrix([[                                                  exp(-lamda_1*t),               0],
                          [alpha*lamda_1/(lamda_1-lamda_2)*(exp(-lamda_2*t)-exp(-lamda_1*t)), exp(-lamda_2*t)]])
...
class TwoPoolsFeedbackSimple(LinearAutonomousPoolModel):
    ...
    def __init__(self, alpha, u_1):
        ...
        Note:
            The symbolic matrix exponential will NOT be initialized automatically.
        """
...
class TwoPoolsFeedback(LinearAutonomousPoolModel):
    ...
    def __init__(self, alpha_12, alpha_21, u_1, u_2):
        ...
        Args:
            alpha_12 (in [0,1] or SymPy expression): proportion of outflow from pool 2 that goes to pool 1
            alpha_21 (in [0,1] or SymPy expression): proportion of outflow from pool 1 that goes to pool 2
            u_1 (nonnegative or SymPy expression): external input rate to pool 1
            u_2 (nonnegative or SymPy expression): external input rate to pool 2
