...
class LinearAutonomousPoolModel(object):
    ...
    def __init__(self, u, B, force_numerical=False):
        ...
        if B.det() == 0:
            ...
            raise NonInvertibleCompartmentalMatrix(
                f"""
            The matrix B is not invertible: {B}
    ...
    def _get_Qt(self, x):
        ...
        if x is None:
            ...
            if not hasattr(self, "Qt"):
                raise (Error("No matrix exponential given for symbolic calculations"))
            ...
            if hasattr(self, "Qt"):
                ...
                B = self.B
                D = np.array(
                    [[B[i, j] for j in range(B.cols)] for i in range(B.rows)],
                    dtype="float64",
                ) * float(x)
                Qt = expm(D)
    ...
    def r_compartments(self):
        ...
        return Matrix(
            self.xss.rows,
            1,
            [phase_type.z(self.B)[i] * self.xss[i] for i in range(self.xss.rows)],
        )
